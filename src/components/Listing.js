...
const StorageCard = ({ listing }) => {
  ...
  return (
    <div
      ...
      onClick={() => {
        updateCurrListing(listing);
        setTimeout(function() {
          document.getElementById("dtView").classList.add("show");
        }, 0);
      }}
    >
      <Card style={{ borderRadius: "8px" }}>
        <Card.Image>
          <Image.Container size="4by3">
            <Image
              alt="Picture of the storage space"
              src="https://i.pinimg.com/originals/6a/7c/fc/6a7cfc513ee281ac19ed5b25f17a9a5a.jpg"
        ...
        <Card.Content style={{ padding: "10px" }}>
          ...
          >
            ...
            >
              <img
                style={
                  listing.attributes.hasElevator
                    ? { height: "22px" }
                    : { display: "none" }
                }
                src={elevator}
          ...
          <table style={{ width: "100%", marginTop: "10px" }}>
            <tr>
              ...
              >
                ...
                >
                  {distanceCalculator(
                    { latitude: 42.057923, longitude: -87.675918 },
                    listing.location.geodata.coordinates
                  )}
                </div>
                <br></br>
              </th>
...
const ListingList = testitems => {
  //const { listingList } = useContext(ListingContext);

  var columnIds = [...Array(testitems.length).keys()];

  return (
    <div>
      <Column.Group multiline data-testid="grid">
        {columnIds.map(i => (
          <Column key={i} size="one-quarter">
            <StorageCard listing={testitems[i]} />
          </Column>
        ))}
      </Column.Group>
    </div>
